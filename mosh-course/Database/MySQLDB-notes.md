...
3.  Select Operation :
    ...
    - Select Query WHERE CLAUSE Logical Operator
      ...
      - `select * from books where (category = 'technology' or category = 'mystery') and price < 30;` -> combination of multiple and, or using bracket () : subquery
    ...
    - Select Query WHERE CLAUSE Pattern Matching
      ...
      - `select * from books where category in ('Technology', 'Mystery', 'Astronomy')` -> it will fetch the values which match the suggested options.
